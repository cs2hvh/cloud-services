/**
 * Vite + React Pipeline - React with Vite build tool
...
<flow-definition plugin="workflow-job@2.44">
  <actions/>
  <description>
    Vite + React deployment pipeline for ${name}
    ...
    stage('Initialize') {
      steps {
        script {
          ...
          echo 'PIPELINE: Vite React Deployment Pipeline'
            ...
            sh(
              script: '''
                ...
                if [ -f vite.config.js ] || [ -f vite.config.ts ]; then
                  ...
                  echo 'WARNING: No vite.config.js/ts found, using default Vite settings'
                fi
