...
function nodeDockerfile(i: DockerfileInput): string {
  ...
  /*
    NEXT.JS output:'standalone' MOVES THE SERVER.

    next build writes a self-contained app to .next/standalone, and detection
    sets the start command to `node server.js` accordingly. Copying /app
    wholesale leaves that file at /app/.next/standalone/server.js, so the
    container started, failed with MODULE_NOT_FOUND on '/app/server.js', and
    crash-looped — while the deployment still reported ready.

    Next deliberately omits static assets and public/ from that directory
    (they are served from disk, not bundled), so both are copied separately.
    This is Vercel's own documented layout for containerising a standalone
    build.
  */
  const standalone = d.standalone === true;

  // `public/` is optional in a Next.js app and COPY fails on a missing source,
  // so guarantee the directory exists rather than making the copy conditional
  // on something this stage cannot see.
  const ensurePublic = standalone ? "\nRUN mkdir -p /app/public" : "";

  const runnerCopy = standalone
    ? [
...
COPY . .
${args}${build ? buildRun(runCmd(pm, build)) : "# no build step for this framework"}${ensurePublic}
 ...
 * PHP.
 *
 * APACHE WITH mod_php, deliberately. php-fpm behind nginx means two processes
 * in one container and a supervisor to keep them both alive, which is a lot of
 * moving parts for no gain here; the CLI server is not a production server.
 * Apache is one process, and Laravel and Symfony both ship .htaccess files it
 * reads without further configuration.
 *
 * THE HARD PART IS NOT PHP, IT IS RUNNING UNPRIVILEGED. Apache normally starts
 * as root, binds port 80 and drops to www-data. This container never gets to
 * be root, so all three of those assumptions have to be undone: the listener
 * moves to 8080, the pid and lock files move somewhere writable, and the
 * directories Apache opens at startup are handed to the uid that will own
 * them. Every static site on this platform once crash-looped because exactly
 * one of those details — an nginx pidfile — was missed, and the symptom was a
 * 503 with a clean startup log in front of it.
 *
 * THE DOCUMENT ROOT IS DECIDED AT BUILD TIME, not guessed here. Laravel and
 * Symfony serve from public/; a plain PHP application usually serves from its
 * root. Testing for public/index.php is the difference between serving an
 * application and serving its source code, which is a security question and
 * not a cosmetic one.
 */
function phpDockerfile(i: DockerfileInput): string {
  const d = i.detection;
  const uid = runtimeUid(d);
  // Symfony only. See the detector for why this is not set for every PHP app.
  const symfony =
    d.framework === "symfony"
      ? "\nENV APP_ENV=prod\nENV APP_DEBUG=0"
      : "";
  ...
  return `# syntax=docker/dockerfile:1.7
# Generated by AhuraCloud deploy v2 — framework: php
FROM composer:2 AS vendor${symfony}
WORKDIR /app
COPY . .
# --ignore-platform-reqs: the vendor stage resolves dependencies, it does not
# run the application, and failing here for an extension the RUNTIME has is a
# false negative. A genuinely missing extension still fails at boot, where the
# error names it.
...
FROM php:8.3-apache AS runner${symfony}
RUN set -eux; \\
    apt-get update; \\
    apt-get install -y --no-install-recommends libpq-dev libzip-dev libicu-dev; \\
    docker-php-ext-install -j"$(nproc)" opcache pdo_mysql pdo_pgsql zip intl; \\
    rm -rf /var/lib/apt/lists/*
RUN a2enmod rewrite
COPY --from=vendor --chown=${uid}:${uid} /app /var/www/html
# Unprivileged Apache: an unprivileged port, writable runtime paths, and logs
# to the container's own stdout rather than files nobody will ever read.
RUN set -eux; \\
    sed -ri 's/^Listen 80$/Listen 8080/' /etc/apache2/ports.conf; \\
    sed -ri 's/:80>/:8080>/' /etc/apache2/sites-available/000-default.conf; \\
    printf 'PidFile /tmp/apache2.pid\\nMutex file:/tmp\\nErrorLog /dev/stderr\\nTransferLog /dev/stdout\\n' > /etc/apache2/conf-available/ahura.conf; \\
    a2enconf ahura; \\
    if [ -f /var/www/html/public/index.php ]; then root=/var/www/html/public; else root=/var/www/html; fi; \\
    echo "document root: $root"; \\
    sed -ri "s!DocumentRoot /var/www/html!DocumentRoot $root!" /etc/apache2/sites-available/000-default.conf; \\
    printf '<Directory %s>\\n  AllowOverride All\\n  Require all granted\\n</Directory>\\n' "$root" >> /etc/apache2/apache2.conf; \\
    mkdir -p /var/run/apache2 /var/lock/apache2 /var/log/apache2; \\
    chown -R ${uid}:${uid} /var/www/html /var/run/apache2 /var/lock/apache2 /var/log/apache2
...
WORKDIR /src
# gcc and musl-dev because the build below enables cgo. See the note there.
RUN apk add --no-cache gcc musl-dev
COPY go.mod go.su[m] ./
RUN --mount=type=cache,target=/go/pkg/mod go mod download
COPY . .
# CGO IS ON, AND THE BINARY IS STILL STATIC. Those are usually opposites.
#
# This built CGO_ENABLED=0, which is the ordinary way to get a binary that runs
# on distroless — and it silently excludes every dependency with a C part.
# github.com/mattn/go-sqlite3 is the common one: it compiles, and then the
# driver is simply absent at run time, so the application dies at startup
# saying sql: unknown driver "sqlite3". gothinkster's Gin app failed exactly
# that way, having built and routed perfectly.
#
# The builder is alpine, so the C library is musl, and musl links statically
# without the caveats glibc has. -linkmode external hands the final link to gcc
# and -extldflags -static tells it to resolve everything in; osusergo and netgo
# keep the user and DNS lookups on Go's own implementations rather than pulling
# in the parts of libc that expect to be dynamic.
#
# A pure-Go module is unaffected: enabling cgo does not change how Go code is
# compiled, only what it is allowed to call.
