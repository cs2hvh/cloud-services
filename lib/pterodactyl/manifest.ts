...
export const getPterodactylGameConfig = (
  name: string,
  user: number,
  game: string,
  memory: number,
  ...
  switch (game.toLowerCase()) {
    ...
    case "cs2":
      return {
        ...
        startup:
          "./game/cs2.sh -dedicated +ip 0.0.0.0 -port {{SERVER_PORT}} +map {{SRCDS_MAP}} -maxplayers {{SRCDS_MAXPLAYERS}} +sv_setsteamaccount {{STEAM_ACC}}",
        environment: {
          SRCDS_MAP: "de_dust2",
          SRCDS_APPID: "730",
          SRCDS_MAXPLAYERS: "64",
    ...
    case "rust":
      return {
        ...
        startup:
          './RustDedicated -batchmode +server.ip 0.0.0.0 -port {{SERVER_PORT}} +server.level "{{RUST_LEVEL}}" +server.hostname "{{HOSTNAME}}" +server.identity "{{SERVER_IDENTITY}}" +server.seed {{WORLD_SEED}} +rcon.port {{RCON_PORT}} +rcon.password "{{RCON_PASSWORD}}" +rcon.web 1',
        environment: {
    ...
    case "valheim":
      return {
        ...
        startup:
          './valheim_server.x86_64 -name "{{SERVER_NAME}}" -port {{SERVER_PORT}} -world "{{WORLD_NAME}}" -password "{{SERVER_PASS}}" -public {{PUBLIC}}',
        environment: {
