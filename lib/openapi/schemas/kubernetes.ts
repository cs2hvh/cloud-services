...
export const KubernetesClusterSchema = z.object({
  ...
  workers: z.array(WorkerNodeSchema).openapi({ description: 'Worker nodes' }),
  ...
  status: z.enum(['pending', 'provisioning', 'ready', 'error', 'deleting', 'deleted']).openapi({ example: 'ready', description: 'Cluster status' }),
