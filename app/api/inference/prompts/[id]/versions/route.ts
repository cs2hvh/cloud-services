...
export async function GET(
  ...
  const rl = await limitByUser(auth.user!.id, { prefix: "rl:prompt-versions-list", limit: 60, windowMs: 60_000 });
...
export async function POST(
  ...
  const rl = await limitByUser(auth.user!.id, { prefix: "rl:prompt-version-create", limit: 30, windowMs: 60_000 });
  if (!rl.allowed) return NextResponse.json({ error: "Too Many Requests" }, { status: 429 });
