  ...
  try {
    ...
    const availableNodes = nodes.filter((node) => {
      const isCorrectLocation =
        node.attributes.location_id === Number(location);
    ...
    if (availableNodes.length > 0) {
      ...
      const allocationsResponse = await ptero_axios.get(
      ...
      const unassignedAllocation = allocations.find(
      ...
      const payload = getPterodactylGameConfig(
        name,
        1,
        game_type,
      ...
      // console.log(payload)
      const response = await ptero_axios.post(
      ...
      return new Response(`Successfully created ${name} server.`, {
        status: 200,
      });
    } else {
