import { LoadingSpinner } from "@/components/dashboard/utils/loading";
import { Projects, Spectrum_Apps, Users } from "@/lib/supabase/queries";
import { notFound } from "next/navigation";
import { Suspense } from "react";
import { requireAdmin } from "@/lib/supabase/auth";
import SpectrumAppCreate from "@/components/dashboard/network-ddos/new";


export const dynamic = "force-dynamic";
const AdminSpectrumAssignSuspense = async () => {
  // Check admin authentication
  const checkAdmin = await requireAdmin();
  
  if (!checkAdmin.ok) {
    notFound();
  }

  // Fetch required data
  const projects = await Projects.get_all_for_admin();
  const usersData = await Users.get_all_profiles();

  // Map users to the format expected by SpectrumAppCreate
  const allUsers = usersData.map(user => ({
    id: user.id,
    email: user.email || "",
    username: user.username || undefined,
  })).filter(user => user.email); // Only include users with email

   const spectrumApps = await Spectrum_Apps.get_all_app_name('admin');

  return (
    <SpectrumAppCreate 
      projects={projects} 
      userId={checkAdmin.userId || ""} 
      role="admin" 
      allUsers={allUsers} 
      spectrumApps={spectrumApps}
    />
  );
};

const AdminSpectrumAssignPage = () => {
  return (
    <div className="flex-1 bg-black min-h-screen p-6 sm:p-8 text-white">
      <div className="mb-8">
        <h1 className="text-3xl font-bold">Assign DDoS Protection to User</h1>
        <p className="text-white/60">
          Create and assign network DDoS protection to any user with advanced Layer 4 security.
        </p>
      </div>
      
      <div className="border-t border-white/10 pt-8">
        <Suspense fallback={
          <div className="flex items-center justify-center py-20">
            <LoadingSpinner />
          </div>
        }>
          <AdminSpectrumAssignSuspense />
        </Suspense>
      </div>
    </div>
  );
};

export default AdminSpectrumAssignPage;
