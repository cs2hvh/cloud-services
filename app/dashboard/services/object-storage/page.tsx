    ...
    if (bucket.endpoint && encryptionKey) {
      try {
        if (bucket.endpoint.startsWith("{")) {
          ...
          decryptedBucket.endpoint = Encryption.decrypt(
            encryptedData,
            encryptionKey
          );
        }
        ...
        console.error(
          `Error decrypting endpoint for bucket ${bucket.id}:`,
          error
        );
    ...
    if (bucket.key_id && encryptionKey) {
      try {
        if (bucket.key_id.startsWith("{")) {
          ...
          decryptedBucket.key_id = Encryption.decrypt(
            encryptedData,
            encryptionKey
          );
        }
        ...
        console.error(
          `Error decrypting key_id for bucket ${bucket.id}:`,
          error
        );
      }
    }
    ...
    if (bucket.secret_key && encryptionKey) {
      try {
        if (bucket.secret_key.startsWith("{")) {
          ...
          decryptedBucket.secret_key = Encryption.decrypt(
            encryptedData,
            encryptionKey
          );
        }
        ...
        console.error(
          `Error decrypting secret_key for bucket ${bucket.id}:`,
          error
        );
      }
    }
