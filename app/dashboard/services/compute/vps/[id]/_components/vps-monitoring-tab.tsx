...
export function VpsMonitoringTab({
  ...
  const gauges = [
    { label: 'CPU', value: met.cpu, unit: '%', color: met.cpu > 80 ? '#ef4444' : met.cpu > 50 ? '#f59e0b' : '#06b6d4' },
    { label: 'Memory', value: met.mem_pct, unit: '%', color: met.mem_pct > 85 ? '#ef4444' : met.mem_pct > 60 ? '#f59e0b' : '#3b82f6' },
  ];
  const ioCards = [
    { label: 'Network In', value: formatBytes(met.net_in), icon: ArrowUpDown, color: 'text-violet-400' },
    { label: 'Network Out', value: formatBytes(met.net_out), icon: ArrowUpDown, color: 'text-purple-400' },
    { label: 'Disk Read', value: formatBytes(met.disk_read), icon: HardDrive, color: 'text-emerald-400' },
    { label: 'Disk Write', value: formatBytes(met.disk_write), icon: HardDrive, color: 'text-teal-400' },
  ];
            ...
            <div key={g.label} className="border border-white/[0.06] rounded-lg bg-[#08080a] p-6 flex items-center gap-6">
              <div className="relative flex-shrink-0">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-2xl font-bold text-white tabular-nums">{g.value.toFixed(1)}</span>
        ...
        {ioCards.map((c) => {
          ...
          return (
            <div key={c.label} className="border border-white/[0.06] rounded-lg bg-[#08080a] p-4">
              ...
              <p className="text-lg font-bold text-white tabular-nums">{c.value}</p>
            </div>
          );
        })}
      </div>
      ...
      <div className="border border-white/[0.06] rounded-lg bg-[#08080a] p-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-white tabular-nums">{met.uptime > 86400 ? `${Math.floor(met.uptime / 86400)}d ` : ''}{Math.floor((met.uptime % 86400) / 3600)}h {Math.floor((met.uptime % 3600) / 60)}m</p>
          </div>
        </div>
      </div>
    </div>
