...
const SingleDbSuspense = async ({databaseId}:{ databaseId: string }) => {
  return <Singledb databaseId={databaseId}  />
};
...
const SingleDbPage = async ({
  ...
  //const databaseId =  decodeURIComponent(databaseId);
  return (
    <div className="flex-1 bg-black min-h-screen p-6 sm:p-8 text-white">
      ...
      <div className="border-t border-white/10 pt-8">
        <Suspense fallback={
          ...
          <SingleDbSuspense databaseId={decodeURIComponent(databaseId)} />
        </Suspense>
      </div>
    </div>
  );
