...
/**
 * DepthText — layered 3D text that tilts with the pointer and orbits gently
 * when it is left alone.
 *
 * Adapted from React Bits (https://reactbits.dev/text-animations/depth-text,
 * https://github.com/DavidHDev/react-bits, MIT + Commons Clause, © David Haz).
 * Changes from the original: multi-line text (a "\n" in `text` becomes a line
 * break, and the layers stack as a block), `letterSpacing` and `lineHeight`
 * props so it can take the hero's type instead of its own, and a "use client"
 * directive. The mechanics (stacked translateZ layers with color-mix, pointer
 * tracking with smoothing, auto-orbit fallback, reduced-motion opt-out) are
 * the original's.
 */
