...
export interface LinkWizardState {
  step: WizardStep;
  source: 'existing' | 'create' | null;
  
  // For existing database selection
  selectedDatabaseId: string | null;
  selectedDatabase: AvailableDatabase | null;
  
  // For creating new database
  createData: CreateDatabaseData | null;
  createdDatabaseId: string | null;
  createdConnectionInfo: DatabaseConnectionInfo | null;
  
  // Environment variable configuration
  envVarConfigs: EnvVarConfig[];
  
  // Force overwrite existing vars
  force: boolean;
  conflicts: string[];
