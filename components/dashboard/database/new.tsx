...
import { CheckCircle2, ChevronRight, Cpu, Database, HardDrive, Loader2, MapPin, Server } from "lucide-react";
...
const DatabaseSelect = ({ products, locations }: PageProps) => {
    ...
    useEffect(() => {
        if (state.selectedDbType) {
            const filteredProducts = products.filter(product => product.sub === state.selectedDbType);
            setAvailablePlans(filteredProducts);

            // Set versions based on selected DB type
            setState(prevState => ({
                ...prevState,
                versions: databaseVersions[state.selectedDbType as keyof typeof databaseVersions] || [],
                selectedVersion: databaseVersions[state.selectedDbType as keyof typeof databaseVersions]?.[0] || "",
            }));
        }
    ...
    const handleDbTypeChange = (dbType: string) => {
        setState(prevState => ({
            ...
            selectedVersion: databaseVersions[dbType as keyof typeof databaseVersions]?.[0] || "",
        }));
    };
    ...
    const selectedLocationData = locations?.find(location => location.short === selectedLocation);
    ...
    const dbTypes = [...new Set(products
                    ...
                    {currentStep === 1 && (
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Choose a unique name for your database cluster</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <Input
                                    value={selectedName}
                                    onChange={(e) => setState(prevState => ({ ...prevState, selectedName: e.target.value }))}
                                    type="text"
                                    placeholder="my-production-db"
                                    className="text-base"
                                />
                                <p className="text-xs text-muted-foreground mt-2">
                                    The name will be used to identify your database in the dashboard
                                </p>
                            </CardContent>
                    ...
                    {currentStep === 2 && (
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Choose a datacenter region for your database</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <RadioGroup
                    ...
                    {currentStep === 3 && (
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Select your database engine type</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <RadioGroup
                                    ...
                                    {dbTypes.map(dbType => (
                                        <div key={dbType}>
                                            ...
                                            >
                                                ...
                                                <div>
                                                    <p className="text-base font-semibold capitalize">{dbType}</p>
                                                    <p className="text-xs text-muted-foreground">
                                                        {dbType === 'mysql' && 'Reliable, popular SQL database'}
                                                        {dbType === 'postgresql' && 'Advanced open-source database'}
                                                        {dbType === 'mongodb' && 'NoSQL document database'}
                                                        {dbType === 'redis' && 'In-memory data structure store'}
                                                        {dbType === 'mariadb' && 'Community-developed MySQL fork'}
                                                        {dbType === 'clickhouse' && 'Column-oriented OLAP database'}
                                                    </p>
                                                </div>
                                                <div className="ml-auto">
                                                    <Badge variant="outline">
                                                        {products.filter(p => p.sub === dbType).length} plans
                                                    </Badge>
                                                </div>
                                            </Label>
                                        </div>
                    ...
                    {currentStep === 4 && (
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Select your {selectedDbType} database plan</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <RadioGroup
                                    value={selectedDb}
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex gap-2 text-xs text-muted-foreground">
                                                            ...
                                                            <span>{database.resources.storage} GB Storage</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div className="text-right">
                                                    <div className="font-semibold">
                                                        {database.price === 0
                                                            ...
                                                            : database.discount
                                                                ? <div>
                                                                    <span className="line-through text-xs text-muted-foreground">{formatPrice(database.price)}/mo</span>
                                                                    <span className="ml-1">{formatPrice(database.price * (1 - Number(database.discount) / 100))}/mo</span>
                                                                </div>
                    ...
                    {currentStep === 5 && selectedDb && (
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Select version for your {selectedDbType} database</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-6">
                                <div>
                                    <Label htmlFor="version" className="mb-2 block">Database Version</Label>
                                    <Select
                                        value={selectedVersion}
                                        onValueChange={(value) => setState(prevState => ({ ...prevState, selectedVersion: value }))}
                                    >
                                    ...
                                    {selectedDatabase && (
                                        <div className="grid grid-cols-3 gap-4 bg-gray-50 dark:bg-secondary rounded-md p-4">
                                            <div className="text-center p-2">
                                                ...
                                                <div className="mt-1 font-medium">{selectedDatabase.resources.cpu} vCPU</div>
                                                <div className="text-xs text-muted-foreground">Dedicated cores</div>
                                            </div>
                                            <div className="text-center p-2">
                                                <div className="flex justify-center">
                                                    <Server className="h-5 w-5 text-muted-foreground" />
                                                </div>
                                                <div className="mt-1 font-medium">{selectedDatabase.resources.ram} GB</div>
                                            ...
                                            <div className="text-center p-2">
                                                ...
                                                <div className="mt-1 font-medium">{selectedDatabase.resources.storage} GB</div>
                    ...
                    {currentStep === 6 && (
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Review your database configuration and complete the order</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-6">
                                <div className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-2">
                                            <h3 className="text-sm font-medium text-muted-foreground">Database Details</h3>
                                            <div className="bg-gray-50 dark:bg-secondary rounded-md p-4 space-y-3">
                                                ...
                                                <div className="flex justify-between">
                                                    ...
                                                    <span className="font-medium capitalize">{selectedDbType}</span>
                                                </div>
                                                <div className="flex justify-between">
                                        ...
                                        <div className="space-y-2">
                                            <h3 className="text-sm font-medium text-muted-foreground">Deployment Location</h3>
                                            <div className="bg-gray-50 dark:bg-secondary rounded-md p-4 space-y-3">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="font-medium">{selectedLocationData?.city}, {selectedLocationData?.country}</span>
                                                    </div>
                                                </div>
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <div className="flex items-center gap-1">
                                                        <MapPin size={14} />
                                                        <span className="font-medium">{selectedLocationData?.short}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <div className="space-y-4">
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <div className="flex items-center gap-1">
                                            <Image
                                                ...
                                                height={12}
                                            />
                                            <span className="font-medium">{selectedLocationData.city}</span>
                                        </div>
                                    </div>
                                )}
                                ...
                                {selectedDatabase && (
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="font-medium text-xs">
                                            {selectedDatabase.resources.cpu} vCPU / {selectedDatabase.resources.ram} GB / {selectedDatabase.resources.storage} GB
                                        </span>
                                    </div>
                                )}
                                ...
                                <div className="pt-2">
                                    ...
                                    {selectedDatabase && selectedDatabase.discount && Number(selectedDatabase.discount) > 0 && (
                                ...
                                <div className="bg-primary/5 p-4 rounded-md flex justify-between items-center">
                                    ...
                                    <span className="font-bold text-lg">
                                        {selectedDatabase
                                            ? selectedDatabase.price === 0
                                                ? "Free"
                                                : selectedDatabase.discount && Number(selectedDatabase.discount) > 0
                            ...
                            <div className="text-xs text-muted-foreground">
                                Databases are billed on a monthly basis. You can cancel at any time.
                            </div>
                        </CardContent>
