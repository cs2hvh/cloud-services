...
const GameServerGrid = ({ data }: { data: Tables<'game_servers'>[] }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {data.map((server) => (
                <Card key={server.id} className="overflow-hidden transition-all duration-300 hover:shadow-md">
                    <CardHeader>
                        ...
                        <CardDescription className="capitalize">{server.game_type} Server</CardDescription>
                    </CardHeader>
                    <Separator />
                    <CardContent>
                        {/* Resources */}
                    ...
                    <CardFooter className="pt-2 flex gap-2">
                        <Button
                            className="w-1/2"
                            variant="secondary"
                            onClick={() => window.open(`https://panel.hav0k.dev/server/${server.identifier}`, '_blank')}
                        >
                            Open Control Panel
