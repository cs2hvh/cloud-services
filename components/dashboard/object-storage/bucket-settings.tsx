...
const BucketSettings = ({ bucket }: BucketSettingsProps) => {
  ...
  const SettingsCard = ({ 
    ...
    >
      <Card className={`bg-gradient-to-br from-${gradientFrom} to-${gradientTo} border-${borderColor} h-fit`}>
        <CardHeader className="pb-3">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
            <div className="flex items-start gap-3 min-w-0 flex-1">
              <div className={`h-8 w-8 sm:h-10 sm:w-10 bg-${iconColor.replace('400', '500/20')} rounded-lg flex items-center justify-center flex-shrink-0`}>
                <Icon className={`h-4 w-4 sm:h-5 sm:w-5 text-${iconColor}`} />
              </div>
              <div className="min-w-0 flex-1">
                <CardTitle className="text-white text-base sm:text-lg leading-tight">{title}</CardTitle>
                <p className="text-white/60 text-xs sm:text-sm mt-1 leading-tight">{description}</p>
              </div>
            </div>
            <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
              {editMode[setting] ? (
                ...
                <Button
                  size="icon"
                  variant="ghost"
                  onClick={() => setEditMode(prev => ({ ...prev, [setting]: true }))}
                  className="h-7 w-7 sm:h-8 sm:w-8 hover:bg-white/10"
                >
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
      ...
      >
        {editMode.acl ? (
          <Select value={settings.acl} onValueChange={(value: 'private' | 'public-read') => setSettings(prev => ({ ...prev, acl: value }))}>
        ...
        {editMode.versioning ? (
          <div className="flex items-center gap-3">
            <Switch
              ...
              onCheckedChange={(checked) => setSettings(prev => ({ ...prev, versioningEnabled: checked }))}
            />
            <span className="text-white/80 text-sm">
      ...
      >
        {editMode.project ? (
          <Select 
            ...
            onValueChange={(value) => setSettings(prev => ({ ...prev, projectId: value === "none" ? null : value }))}
          >
            <SelectTrigger className="bg-white/5 border-white/10 text-white">
              <SelectValue />
            </SelectTrigger>
          ...
          <div className="flex items-center gap-2">
            <span className="px-3 py-1 rounded-full text-sm font-medium bg-yellow-500/20 text-yellow-300 border border-yellow-500/30">
              {isLoadingProjects ? "Loading..." : getProjectName(settings.projectId)}
            </span>
          </div>
        )}
      </SettingsCard>
