...
import { BadgeHelp, CheckCircle2, ChevronRight, Cpu, Gamepad, HardDrive, Loader2, MapPin, Server, Shield, Wifi } from "lucide-react";
...
const GameServerSelect = ({ products, locations }: PageProps) => {
    ...
    const selectedPlan = products.find(product => product.id === state.selectedPlan);

    // Get the selected location details
    const selectedLocation = locations.find(loc => loc.attributes.short === state.selectedLocation);
                    ...
                    {currentStep === 1 && (
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Choose the game you want to host on your server</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <RadioGroup
                                ...
                                >
                                    {Object.entries(GAME_TYPES).map(([key, game]) => (
                                        <div key={key}>
                                            <RadioGroupItem value={key} id={`game-${key}`} className="peer sr-only" />
                                            <Label
                                                htmlFor={`game-${key}`}
                                            ...
                                            >
                                                <div className="flex flex-col">
                                                    ...
                                                    <div className="grid grid-cols-4 gap-3 mt-3">
                                                        <div className="flex flex-col items-center p-2 bg-primary/5 rounded">
                                                            <Cpu size={16} className="text-muted-foreground mb-1" />
                                                            <span className="text-sm font-medium">{plan.resources.cpu} CPU</span>
                                                        </div>
                                                        <div className="flex flex-col items-center p-2 bg-primary/5 rounded">
                                                            <Server size={16} className="text-muted-foreground mb-1" />
                                                            <span className="text-sm font-medium">{plan.resources.ram} GB</span>
                                                        </div>
                                                        <div className="flex flex-col items-center p-2 bg-primary/5 rounded">
                                                            <HardDrive size={16} className="text-muted-foreground mb-1" />
                                                            <span className="text-sm font-medium">{plan.resources.storage} GB</span>
                                                        </div>
                                                        <div className="flex flex-col items-center p-2 bg-primary/5 rounded">
                                                            <Wifi size={16} className="text-muted-foreground mb-1" />
                                                            <span className="text-sm font-medium">{plan.resources.bandwith} GB</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="text-right">
                                                    <div className="font-bold text-lg">
                                                        {formatPrice(plan.price * (1 - plan.discount! / 100))}/mo
                                                    </div>
                                                    {plan.discount! > 0 && (
                    ...
                    {currentStep === 3 && (
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Choose a location for your game server</CardDescription>
                            </CardHeader>
                            <CardContent>
                                <RadioGroup
                                    ...
                                    {locations.map((location) => (
                                        <div key={location.attributes.id}>
                                            ...
                                            >
                                                ...
                                                <div>
                                                    <div className="font-medium">{location.attributes.long}</div>
                                                    <div className="text-xs text-muted-foreground">Region: {location.attributes.short}</div>
                                                </div>
                                            </Label>
                                        </div>
                                    ))}
                    ...
                    {currentStep === 4 && (
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Customize your game server settings</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-6">
                                <div>
                                    <Label htmlFor="server-name" className="mb-2 block">Server Name</Label>
                                    <Input
                                        id="server-name"
                                        value={state.serverName}
                                        onChange={(e) => setState(prev => ({ ...prev, serverName: e.target.value }))}
                                        placeholder="My Awesome Game Server"
                                        className="text-base"
                                    />
                                    <p className="text-xs text-muted-foreground mt-2">
                                        This name will be displayed in the server browser and your dashboard
                                    </p>
                                </div>

                                <div>
                                    <Label htmlFor="project-id" className="mb-2 block">Project</Label>
                                    <Select value={state.selectedProject} onValueChange={(e) => setState(prev => ({ ...prev, selectedProject: e }))}>
                                        <SelectTrigger>
                                ...
                                <div>
                                    <h3 className="text-base font-medium mb-4">Additional Services</h3>
                                    <div className="space-y-4">
                                        {ADDITIONAL_SERVICES.map(service => (
                                            <div key={service.id} className="flex items-center justify-between">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <span className="text-sm font-medium">{formatPrice(service.price)}/mo</span>
                                                    <Switch
                                                        checked={state.additionalServices[service.id] || false}
                    ...
                    {currentStep === 5 && (
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Review your game server configuration and complete the order</CardDescription>
                            </CardHeader>
                            <CardContent className="space-y-6">
                                <div className="space-y-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-2">
                                            <h3 className="text-sm font-medium text-muted-foreground">Server Details</h3>
                                            <div className="bg-gray-50 dark:bg-secondary rounded-md p-4 space-y-3">
                                                ...
                                                <div className="flex justify-between">
                                                    ...
                                                    <span className="font-medium">{GAME_TYPES[state.selectedGameType as GameType]?.name}</span>
                                                </div>
                                                <div className="flex justify-between">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="bg-gray-50 dark:bg-secondary rounded-md p-4 space-y-3">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <MapPin size={14} />
                                                        <span className="font-medium">{selectedLocation?.attributes.long}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div className="bg-gray-50 dark:bg-secondary rounded-md p-4">
                                                <div className="grid grid-cols-4 gap-4">
                                                    <div className="text-center p-2">
                                                        ...
                                                        <div className="mt-1 font-medium">{selectedPlan.resources.cpu} vCPU</div>
                                                    ...
                                                    <div className="text-center p-2">
                                                        ...
                                                        <div className="mt-1 font-medium">{selectedPlan.resources.ram} GB</div>
                                                    ...
                                                    <div className="text-center p-2">
                                                        ...
                                                        <div className="mt-1 font-medium">{selectedPlan.resources.storage} GB</div>
                                                    ...
                                                    <div className="text-center p-2">
                                                        ...
                                                        <div className="mt-1 font-medium">{selectedPlan.resources.bandwith} GB</div>
                                    ...
                                    {Object.keys(state.additionalServices).some(key => state.additionalServices[key]) && (
                                        <div className="space-y-2">
                                                ...
                                                {ADDITIONAL_SERVICES.map(service => (
                                                    state.additionalServices[service.id] && (
                                                        <div key={service.id} className="flex justify-between">
                                                            ...
                                                            <span className="font-medium">{formatPrice(service.price)}/mo</span>
                                                        </div>
                                                    )
                                                ))}
                                            </div>
                    ...
                    <Card className="sticky top-8">
                        ...
                        <CardContent className="space-y-6">
                                ...
                                {selectedPlan && (
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="font-medium text-xs">
                                            {selectedPlan.resources.cpu} vCPU / {selectedPlan.resources.ram} GB / {selectedPlan.resources.storage} GB
                                        </span>
                                    </div>
                                )}
                                            ...
                                            {ADDITIONAL_SERVICES.map(service => (
                                                state.additionalServices[service.id] && (
                                                    <div key={service.id} className="flex justify-between items-center">
                                                        ...
                                                        <span className="text-sm">{formatPrice(service.price)}/mo</span>
                                                    </div>
                                                )
                                            ))}
                                        </div>
                            ...
                            <div className="text-xs text-muted-foreground">
                                Game servers are billed on a monthly basis. You can cancel at any time.
                            </div>
                        </CardContent>
