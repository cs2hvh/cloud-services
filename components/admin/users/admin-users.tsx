...
export default function AdminUsers({ all_users }: PageProps) {
  const router = useRouter();

  const USERS_PER_PAGE = 10;

  const [users, setUsers] = useState(all_users.slice(0, USERS_PER_PAGE));
  const [loading, setLoading] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [roleFilter, setRoleFilter] = useState<"users" | "admin" | "member">("users");
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(
    Math.ceil(all_users.length / USERS_PER_PAGE)
  );
  ...
  const getFilteredUsers = () => {
    ...
    // if roleFilter === 'users', don't filter by role (show all)
  ...
  const handleRoleFilterChange = (value: "users" | "admin" | "member") => {
    setRoleFilter(value);
    // Need to use setTimeout to ensure state is updated before pagination
   // setTimeout(() => updatePagination(1), 0);
  };
  ...
  const handleSuspendToggle = async (user: Admin_User) => {
    setEditingUser(user);
    setUpdating(true);
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-1.5">
                        ...
                        <Button
                          ...
                          title={
                            user.suspend ? "Activate user" : "Suspend user"
                          }
                        >
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                <StatCard
                  icon={Server}
                  label="Servers"
                  value={Number(selectedUser.server_counts)}
                />
                <StatCard
                  icon={Gamepad2}
                  label="Game Servers"
                ...
                <StatCard
                  icon={DatabaseIcon}
                  label="Clusters"
                  value={Number(selectedUser.db_counts)}
                />
                <StatCard
                  icon={FolderOpen}
                  label="Projects"
      ...
      <Dialog open={isRoleDialogOpen} onOpenChange={setIsRoleDialogOpen}>
        <DialogContent className="bg-neutral-900 border-neutral-800 text-white">
          <DialogHeader>
            ...
            <DialogDescription className="text-neutral-500 text-sm">
              Update roles for {editingUser?.username || "user"}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-3 py-4">
            {ROLE_OPTIONS.map((role) => {
              ...
              const wasChecked = (editingUser?.roles as string[] | undefined)?.includes(role.value) || false;
