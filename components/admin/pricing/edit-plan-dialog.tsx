  ...
  useEffect(() => {
    if (product) {
      setFormData({
       ...
       summary: {
  billing: product.summary?.billing ?? "Monthly",
  support: product.summary?.support ?? "Standard",
  provisioning: product.summary?.provisioning ?? "Instant",
  guarantee: product.summary?.guarantee ?? "60 Days",
  buttonText: product.summary?.buttonText ?? "Get Started",
},
