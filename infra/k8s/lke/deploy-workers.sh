...
# ft-runner + deploy-runner + eval-runner (+ in-cluster Redis) to an EXISTING LKE cluster.
#
...
EVAL_IMAGE="$REGISTRY/ahura-eval-runner:$TAG"
...
DEFAULT_EVAL="ghcr.io/cs2hvh/ahura-eval-runner:latest"

say() { printf '\n▸ %s\n' "$*"; }
die() { printf 'ERROR: %s\n' "$*" >&2; exit 1; }

say "deploy-workers — registry=$REGISTRY tag=$TAG skip_build=$SKIP_BUILD (ft + deploy + eval)"
...
if [[ "$SKIP_BUILD" != "1" ]]; then
  ...
  docker build -t "$FT_IMAGE" -f "$REPO_ROOT/workers/ft-runner/Dockerfile" "$REPO_ROOT/workers"
  say "building $DEPLOY_IMAGE"
  docker build -t "$DEPLOY_IMAGE" -f "$REPO_ROOT/workers/deploy-runner/Dockerfile" "$REPO_ROOT/workers"
  say "building $EVAL_IMAGE"
  docker build -t "$EVAL_IMAGE" -f "$REPO_ROOT/workers/eval-runner/Dockerfile" "$REPO_ROOT/workers"
  ...
  say "SKIP_BUILD=1 — deploying already-published $FT_IMAGE / $DEPLOY_IMAGE / $EVAL_IMAGE"
fi
...
if [[ "$FT_IMAGE" != "$DEFAULT_FT" ]]; then
  say "overriding image refs → $FT_IMAGE / $DEPLOY_IMAGE / $EVAL_IMAGE"
  ...
  kubectl -n ahura set image deploy/ahura-eval-runner   runner="$EVAL_IMAGE"
fi
...
kubectl -n ahura rollout restart deploy/ahura-ft-runner deploy/ahura-deploy-runner deploy/ahura-eval-runner
...
kubectl -n ahura rollout status  deploy/ahura-eval-runner   --timeout=180s
...
Tail logs (expect: "<runner> booting" → "claimer started" intervalMs:5000 → "<runner> ready"):
  ...
  kubectl -n ahura logs -f deploy/ahura-eval-runner

Smoke test:
  Fine-Tuning    → New job (tiny dataset) → ft-runner logs show "claimed"
  Deployments    → New deploy             → deploy-runner logs show "claimed"
  Evals          → New run (small dataset)→ eval-runner logs show "claimed eval run"
...
INFERENCE_PLATFORM_KEY must be a valid platform key with access to all models.
EOF
