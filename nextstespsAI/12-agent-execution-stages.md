...
> - **Honest gaps (as-built):** (1) the `function` webhook tool is **API-only — no builder form in the UI** yet (name + webhook URL + params + optional signing secret). (2) `web_search` is unit-tested but **not verified against real Brave** (no key configured). (3) webhook **HMAC signing** is a noted follow-up. (4) Route/integration coverage exists (`responses.test.ts`, `lifecycle.test.ts`, `tools.test.ts`, Playwright `agents-playground.spec.ts`).
