...
2. ~~Markup/margin metric is structurally broken~~ — **fixed 2026-07-17.** `usage.ts` now computes `upstream_cost_cents` from `models.upstream_pricing` (never off-peak-discounted, falls back to `cost_cents` — not `0` — when a model isn't synced, so "unmeasured" doesn't masquerade as "100% margin"). **Caveat found live: 0 of 86 models in the real DB have `upstream_pricing` populated** — `scripts/sync-or-model-pricing.ts --apply` has apparently never been run. The code fix is real; the numbers stay dark until that script runs, which needs `OPENROUTER_API_KEY` — a call for whoever has it, not run here.
3. **Sandbox brand-scrub gaps** — ~~wrong scrubber on sandbox stdout~~ (fixed 2026-07-17, `26a59d25`, `infra-scrub.ts`), ~~unscrubbed FT training logs~~ (fixed 2026-07-17 — `log-url` route now fetches+scrubs server-side via `stripInfraIdentifiers` instead of presigning the raw object; live-verified against a real R2 object with real RunPod/kubectl/path content), ~~no image EXIF stripping~~ (fixed 2026-07-17 — `workers/inference/src/lib/image-scrub.ts`, pure-byte PNG/JPEG metadata strip, no library — wired into `images.ts` before any generated image leaves the gateway; 9 tests incl. a real known-good minimal PNG fixture and a byte-exact check that entropy-coded JPEG scan data is never touched). **Scope note, stated plainly so it isn't mistaken for done: this strips text/EXIF METADATA, not an imperceptible pixel-level watermark (e.g. SynthID)** — those are engineered specifically to survive this kind of processing; removing one means re-encoding/perturbing actual pixel data, a different and much heavier problem than "EXIF strip." Not attempted.
4. **Four still-forked billing mechanisms** (unified pipeline / FT / GPU-serving / vector) — bigger structural work, not a quick fix.
5. ~~No shared media/blob helper~~ — **resolved by policy, 2026-07-17: generated media is deliberately never persisted on our side.** Confirmed with the user: images/audio/music already always return inline base64 (nothing stored), video already only stores the upstream URL and live-proxies bytes (never persists them to our own storage either). The original "three ad hoc patterns" finding was about code-style inconsistency across the three routes, not a storage/ZDR leak — and with no-persistence confirmed as the actual policy, there's no shared blob helper to build; a helper would exist to manage OUR OWN storage lifecycle, which by policy doesn't happen. Nothing to do here.
...
- **Margin metric**: `SELECT cost_cents, upstream_cost_cents FROM inference.usage ORDER BY created_at DESC LIMIT 20` — every row will still show `upstream_cost_cents = cost_cents` until `upstream_pricing` is synced (§4 item 2's caveat), which is expected post-fix, not a sign the fix didn't land — check `SELECT model_id FROM inference.models WHERE upstream_pricing IS NOT NULL` to see whether any model has real data yet.
...
- **Training-log scrub**: fetch a job's log via the `log-url` route and confirm no `RunPod`/`kubectl`/absolute-path content survives — the raw R2 object itself is unaffected (still has the real content for internal/ops use), only what this route returns is scrubbed.
