...
> **BUILD STATUS (2026-07-07):** **S1 ✅ shipped** (`66cc22d8`, verified live E2E), **S2 hosted tools ✅ shipped** (`93c6a518`; incl. an SSRF guard on function webhooks), **S3 code interpreter 🚧 built (stateful dev sandbox: persistent kernel, session reuse, data image) but hard-gated** behind `SANDBOX_ENABLED` pending the security review in **[13-agent-s3-sandbox-security-review.md](13-agent-s3-sandbox-security-review.md)** — its session lifecycle (DB-persisted `sandbox_sessions`, `settleSandboxSession`, `session-reaper` cron) was audited as missing on 2026-07-04 and is now closed, committed-tested, and live-verified against real Supabase. **S5 agent memory ✅ built + migration applied** (write/search tool, org+agent-scoped, ZDR-gated, purge API). **On-behalf-of billing attribution ✅ built, migration applied, platform key flagged, live-verified end-to-end (2026-07-06, commit `6d894eda`)** — closes a live misattribution the same audit found: agent-runner's single static platform key meant every agent run billed model/tool cost to whichever org owns that key, never the customer. **S4 (MCP) redefined + shipped as a CLIENT (2026-07-07, not yet committed)** — §7's original "S4 MCP" below meant *hosting* (run the customer's container); that's now a separate, still-deferred concern. What shipped is an MCP **client** + registry (connect to a remote server, discover its tools, call them — register once, bind by slug across agents), fully spec'd in **[14-agent-mcp-implementation.md](14-agent-mcp-implementation.md)**: adapter (M1), metering (M2), and the org+curated registry (M3) are built, tested, and live-verified end-to-end against a real MCP server; M4 (curated-catalog seed + management screen) is next. Two migrations pending apply. See doc 14's own BUILD STATUS banners for the full account. Full as-built status + honest gaps: see the BUILD STATUS block in [12-agent-execution-stages.md](12-agent-execution-stages.md). Companion docs: **12** = task board · **13** = S3 sandbox security gate · **14** = MCP client + registry.
...
> **⚠️ SUPERSEDED (2026-07-07) — this `agentcore.mcp_servers` sketch is the HOSTING variant (Service D part b), still deferred, NOT what shipped.** S4 was redefined client-first in **[14-agent-mcp-implementation.md](14-agent-mcp-implementation.md)**: the table that actually shipped (migration `20260707000002_agentcore_mcp_servers.sql`) is control-plane metadata only — `display_name`/`server_url`/`auth_token_enc`/`allowed_tools`/`tool_schemas`, `status` CHECK'd to `('active','error','disabled')` (default `'active'`, not `'provisioning'`), `visibility IN ('private','curated')` (not `'public'`) — **no** `deployment_id`/`endpoint_url`/`hourly_cents`, and **no `billing.active_agent_mcp` at all** (the client bills per-call via a `mcp_call` unit label, not always-on). Same table name, materially different shape — don't confuse the two when reading old vs. new. `billing.active_agent_mcp` above stays exactly as sketched, fully deferred, only relevant if/when hosting (doc 14 §11) is ever built.
...
**Fixed (2026-07-06, same pass) — a second real bug caught only by live-testing `code` after the ingress landed.** `inference.usage.num_units` is `INTEGER`; `code`'s `cpu_seconds` metering is naturally fractional (a fast script reports e.g. `0.0002`). A real `SANDBOX_ENABLED=true` Docker run hit exactly this: the row INSERT failed ("invalid input syntax for type integer") and the usage-events queue **dropped the message after 4 retries** — silent data loss, not a delay, and something no fake-backed unit test could ever have caught (fakes don't enforce real Postgres column types). Fixed with `normalizeNumUnits()` in `usage.ts` (ceils a fractional `numUnits` before it's used for either cost computation or the stored row, keeping the two consistent). 5 new tests; re-verified live afterward. Also worth recording: this same live pass proved `file_search` and `code` correct as *tools*, not just their billing — a real Docker-executed `17!` and a real cited vector-search answer — beyond what any prior status line claimed.
