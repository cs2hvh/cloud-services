...
**Date:** 2026-07-06 · **Companion to:** [11-agent-implementation-plan.md](11-agent-implementation-plan.md) (§4 plugin model, S4) · [02-agent-infrastructure.md](02-agent-infrastructure.md) (Service D) · **Status:** M1–M4 shipped, all 4 migrations applied + live-verified (including an M4 edit-support follow-up and an M4 schema-refresh-cron follow-up). M5 verified manually across many real scenarios, plus an automated L2 test now exists — L3/L4 still not built. M6: OAuth 2.1 built (manual client registration, not DCR), migration applied, and now **proven with a real third-party-shaped OAuth+MCP round trip** (own public server via ngrok, not a mock) — real PKCE, real token issuance, real automatic refresh mid-run, real MCP tool call, real answer back through a real agent run. Connection pooling and MCP hosting remain (optional, deferred).

> **BUILD STATUS (2026-07-10) — M6 follow-up: health-check cron was blind to OAuth — ✅ FIXED.** Found by deliberately re-reading the M4 schema-refresh cron (`mcp-schema-refresh.ts`) against the new OAuth columns instead of assuming it already handled them, prompted by a request to check the new work against prior implementation rather than just the new files in isolation. It didn't: that cron predates M6 and only ever selected/decrypted `auth_token_enc` — every `auth_type: 'oauth'` server would have connected with **no Authorization header at all** on every ~30-minute sweep and been marked `status: 'error'` regardless of whether its OAuth credentials were actually fine. A real, live-impacting gap, not a hypothetical: it would have made every OAuth server in the management UI look permanently broken. Fixed by exporting `resolveOAuthToken` from `mcp-registry.ts` (via a new narrow `OAuthTokenRow` type covering just the fields it needs) and having the cron call the exact same resolve-and-refresh logic the per-run adapter uses — one OAuth resolution path, not two, per the §2b modularity contract. Pending (never-connected) OAuth servers are skipped rather than health-checked (nothing to check yet, and attempting one would just be a second guaranteed spurious failure). 3 new tests (skip-pending, connected-server-reuses-real-refresh-logic, refresh-failure-marks-error-without-crashing-the-sweep) — agent-runner now at 141/141 passing, zero regressions.
>
> **BUILD STATUS (2026-07-10) — M6 self-review pass: 3 more real gaps caught before commit, all fixed.** Found by going file-by-file through all 24 changed files rather than trusting the earlier summary, per the same "check against what already exists, don't just review the new files in isolation" instruction that caught the cron gap above. (1) **`mcp_server.oauth_connected` was added to the Postgres enum but never emitted** — the callback route had no `recordAudit` call at all, and the TS-side `InferenceAuditAction` union didn't even include the new value yet, so the call would have failed to typecheck if attempted. Fixed: added the union member, threaded the initiating user's ID through the signed `state` param (the callback route has no session of its own — see its file header — so this was the only way to attribute the audit entry to a real actor), and added the `recordAudit` call. (2) **The `/oauth/authorize` route had no rate limiting**, unlike every other route in this file family. Fixed with the same `limitByUser` pattern the register/edit/delete routes already use. (3) **The Edit-server dialog never got the OAuth fields** — `openEdit`/`save` already read/wrote `oauth_client_id`/`oauth_client_secret`/`oauth_scope` state, but no `<Input>` ever rendered them, and the dialog unconditionally showed an irrelevant "Auth token" field for OAuth-mode servers. Fixed: the edit dialog now branches on `auth_type` exactly like the create dialog already did.
>
> **BUILD STATUS (2026-07-10) — post-migration live testing: one real, more serious gap found and fixed — no timeout on ANY OAuth discovery/token network call.** After the migration was applied to the live database, live-testing `/oauth/authorize` against a real external domain (`example.com` — a real, reachable server that simply doesn't run an OAuth service) hung the request indefinitely. Proven to be a genuine unbounded hang, not slowness: three consecutive attempts with client timeouts of 30s/90s/150s each took *exactly* as long as the client was willing to wait (29831ms/89989ms/150011ms), rather than converging on a fixed duration — conclusive evidence the underlying call never completes on its own. Root cause: `discoverOAuthServerInfo`/`startAuthorization`/`exchangeAuthorization`/`refreshAuthorization` all take a `fetchFn`, and both SSRF-guard wrappers built for M6 (`lib/mcp/oauth-fetch.ts` on the Next.js side, `mcp-registry.ts`'s `makeGuardedFetch` on the agent-runner side) validated the URL but attached no timeout to the actual `fetch()` call — so a server that's unreachable or simply never responds hangs the request forever. This is worse on the agent-runner side than it first appears: that same code path runs inside a live agent run (would hold a worker's concurrency slot open indefinitely) *and* inside the schema-refresh cron's sequential sweep (would stall every server checked after the stuck one, not just itself). Fixed in both wrappers with a 15s-per-hop `AbortSignal.timeout` (matching `url-ingest.ts`'s existing fetch-timeout convention), merged via `AbortSignal.any` with any signal the caller already passed. Re-tested live immediately after the fix: the same `example.com` authorize request that previously hung past 150s now completes in 3.4s, correctly falls back to treating the server's own origin as the authorization server (per the SDK's documented RFC 9728 fallback behavior), and produces a fully-formed, real, valid OAuth 2.1 authorization URL — real PKCE `code_challenge`, correct `client_id`/`scope`/`resource`, and an unforgeable `state` — proof the entire flow is correct up to (and excluding) the one step that requires a human to actually click "Approve" on a real provider's real consent screen. Also separately live-verified during this same pass: the SSRF guard correctly blocks a real cloud-metadata-IP target (`169.254.169.254`) at the actual discovery-fetch call, not just superficially, and records `oauth_status: 'error'` with the real rejection message; the client secret is genuinely AES-GCM-encrypted at rest (verified the stored ciphertext contains no trace of the plaintext); editing `oauth_client_id`/`oauth_client_secret`/`oauth_scope` works; and a pre-existing static-token MCP server (`deepwiki`) still completes a real agent run correctly post-migration (`mcp:mcp__deepwiki__read_wiki_contents` step, real grounded answer, correctly costed) — zero regression to the 3 rows that existed before this migration.
>
> **Unrelated to this feature, found during the same live session and worth recording:** the local dev environment's long-lived Node processes (agent-runner, and separately the Next.js dev server) were both stuck with a broken IPv6-first DNS resolution path, causing agent-runner's BullMQ claim loop to hang on its very first tick forever (`last_claim_tick_ms_ago` stayed `null` indefinitely, silently no-op-ing every subsequent poll — confirmed NOT a code bug: the identical Supabase query succeeded in <500ms from a fresh standalone script, and direct `curl` to Supabase was instant). Fixed locally by restarting both processes with `NODE_OPTIONS=--dns-result-order=ipv4first`. This is an environment quirk of this specific sandbox, not a product defect — noted here only because it looked, at first, exactly like the timeout bug above and cost real time to tell apart.

> **BUILD STATUS (2026-07-10) — M6 OAuth 2.1 (Authorization Code + PKCE) — ✅ SHIPPED, migration written (not yet applied — this repo's convention is the user applies migrations), unit/integration-tested against a real local auth server, browser click-through not yet done.**
>
> **Scope decision, per §10 decision #2 revisited:** manual client registration only, no RFC 7591 Dynamic Client Registration — the customer creates their own OAuth app on the provider and pastes `client_id`/`client_secret` at registration time, same trust model `byok-keys` already uses. DCR is a strict superset this schema doesn't block adding later.
>
> **What's new:**
> - Migration `20260710000001_agentcore_mcp_oauth.sql` — `auth_type` ('static'|'oauth') alongside the existing token column, plus `oauth_client_id`, `oauth_client_secret_enc`, `oauth_scope`, `oauth_access_token_enc`, `oauth_refresh_token_enc`, `oauth_token_expires_at`, cached `oauth_authorization_server_url`/`oauth_resource_metadata` (avoids re-discovery on every refresh), and `oauth_status`/`oauth_last_error` (mirrors the existing `status`/`last_error` visibility pattern). `auth_type='static'` rows are 100% unaffected — every pre-M6 row keeps working unchanged.
> - **Dashboard-driven consent flow** (OAuth needs a human click; agent-runner is a backend service with no browser): `GET /api/agents/mcp-servers/[id]/oauth/authorize` (session-authed, discovers the server via `discoverOAuthServerInfo`, builds the authorization URL + PKCE via `startAuthorization`, redirects) → provider consent screen → `GET /api/agents/mcp-servers/oauth/callback` (exchanges the code via `exchangeAuthorization`, encrypts + stores the tokens). The PKCE `code_verifier` can't live in server memory across that redirect (Next.js routes are stateless, the browser leaves our origin entirely) — it rides inside a signed, AES-GCM-encrypted, 10-minute-TTL `state` param (`lib/mcp/oauth-state.ts`) instead of a new ephemeral DB table.
> - **Every discovery/token-endpoint URL is SSRF-guarded** (§5's explicit call-out: "OAuth discovery URLs are themselves an SSRF vector") — `lib/mcp/oauth-fetch.ts` on the dashboard side (reuses `url-ingest.ts`'s `assertPublicHttpUrl`), agent-runner's existing `ssrf.ts` on the refresh side — passed as the SDK's `fetchFn` option to every discovery/exchange/refresh call, not a new guard invented for this.
> - **agent-runner now encrypts, not just decrypts** (`mcp-crypto.ts` gained `encryptMcpToken`) — a real, deliberate break from the M3 comment claiming "agent-runner only ever decrypts." OAuth breaks that assumption: an access token expires, and agent-runner — the only process that ever makes a live MCP call, hence the only process that ever discovers expiry — refreshes it mid-run and must persist the new tokens back.
> - **`mcp-registry.ts`'s `resolveRegistryMcpConfig`** now branches on `auth_type`: oauth-mode rows resolve (and silently, best-effort refresh-if-expired) via `resolveOAuthToken`, using the SDK's real `refreshAuthorization` — never rewriting OAuth wire logic by hand. A refresh failure (revoked grant, dead auth server) records `oauth_status:'error'` and returns `null`, so the caller treats it exactly like every other connect failure (§7 scenario 3: skip this server, never fail the run) — the same discipline as the static-token path, not a new failure mode.
> - **UI**: `mcp-servers/page.tsx` gained a Bearer-token/OAuth toggle in the register dialog (client ID/secret/scope fields swap in), an `oauth_status` badge in the row, a "Connect" button that hits the authorize route for `pending`/`error` servers, and a toast reading the callback's `?mcp_oauth=connected|error` redirect param. **Known gap**: the Edit dialog doesn't yet expose oauth_client_id/secret fields (only Create does) — re-registering is the workaround today.
>
> **Verification — what's real vs. not yet:**
> - `tests/unit/mcp/oauth-state.test.ts` (6 tests): the signed-state round-trip, URL-safety, wrong-DEK rejection, tamper rejection, and TTL expiry — all real crypto, no mocks.
> - `mcp-oauth-refresh.integration.test.ts` (5 tests, agent-runner): a **real** hand-rolled OAuth token endpoint (plain `node:http`, no Docker, mirrors `mcp.integration.test.ts`'s no-Docker approach) receiving the SDK's real `refreshAuthorization` POST — proves valid-token passthrough, real refresh-and-persist (decrypts the newly-stored ciphertext back to confirm it, including a rotated refresh token — proves the NEW one was stored, not the old), a revoked-grant failure path, fail-closed-with-no-DEK, and no-refresh-token-available. Confirmed the SSRF guard's dev escape hatch (`allowPrivate`, existing convention from the function-webhook tool) needed threading one level deeper than before — found by the guard correctly blocking the test's own `127.0.0.1` server on the first run, not a bug in the guard.
> - **Not yet done**: no live click-through against a real third-party OAuth provider (no browser-automation tool in this environment, same limitation noted for M1's original live-verify) — the authorize/callback **routes themselves** aren't test-covered yet either, since they call `AgentcoreMcpServers` (real Supabase) and this repo's migrations are user-applied, not run by whoever builds a feature; once the migration is applied, a live click-through is the natural next step. agent-runner: 138/138 passing (up from 133), zero regressions. Next.js: 47/47 relevant unit tests passing. Full `tsc --noEmit` clean on both packages. Zero new lint issues on any changed file.

> **BUILD STATUS (2026-07-10) — M5 L2 protocol-integration test — ✅ SHIPPED, no Docker needed.** §9b/§8 originally called for a Docker-gated reference server (`@modelcontextprotocol/server-everything`, `RUN_MCP_IT=1`). Built differently, better: the SDK we already depend on ships both the client (what `mcp-client.ts` wraps) and the server primitives (`McpServer`, `StreamableHTTPServerTransport`), so `mcp.integration.test.ts` spins up a real MCP server in-process on a random localhost port and points the real, unmodified adapter (`connectMcpTools`/`resolveInlineMcpConfig` from `mcp.ts`) at it over real HTTP — a genuine `initialize → tools/list → tools/call` round-trip, real `content[]`/`isError` response shapes, not a fake. No Docker dependency means this test is **not** opt-in-and-usually-skipped like the sandbox IT suite — it runs by default, every time. 4 tests: full round-trip (list + call + real `inputSchema` parsed off the wire), a real tool-level `isError` surfaced as output, `allowed_tools` filtering against the server's actual tool list, and the HTTPS guard correctly rejecting a plain-`http://` URL when `allowPrivate` isn't set. **One real bug caught building this**, not in the adapter but in the test's own MCP-server wiring: stateless mode (`sessionIdGenerator: undefined`) requires a fresh `McpServer`+`StreamableHTTPServerTransport` pairing per HTTP request (confirmed against the SDK's own `examples/server/simpleStatelessStreamableHttp.js`) — reusing one transport instance across multiple requests (the stateful-mode pattern) silently breaks the second-and-later request. agent-runner now at 133 tests passing, zero regressions, `tsc --noEmit` clean.
...
> **BUILD STATUS (2026-07-11) — M6 real end-to-end OAuth+MCP round trip — ✅ PROVEN LIVE, plus one real deploy-config bug found and fixed.** Built a standalone, real (not mocked) combined OAuth 2.1 authorization server + MCP resource server (real PKCE SHA-256 verification, real token issuance with a deliberately short 20s access-token TTL to force a real refresh, real bearer-token enforcement on `/mcp`), exposed it via a real public `ngrok` HTTPS URL (authorized explicitly by the user first) so the production SSRF guard needed no dev bypass. Registered it as a real MCP server, ran the real dashboard OAuth consent flow end-to-end, confirmed `oauth_status: 'connected'` in the live DB, then ran a real agent against it. **First attempt failed** — the run completed with only a `model` step, no `mcp` step, model reporting no tools available. Root-caused to an environment/deploy gap, not application code: `workers/agent-runner/.run-local.sh` (the local script that boots the actual long-running worker process — gitignored, not part of the staged M6 files) never exported `BYOK_DEK`, so the live worker had `mcpTokenDek: null` and silently fail-closed every OAuth-registry server by design (the exact "dek null → skip" behavior documented in §M3's data flow). A standalone diagnostic script had passed the DEK manually, which is why it kept succeeding while the real worker kept failing — a reminder that a component working in isolation doesn't prove the deployed wiring is correct. Fixed with a one-line `export BYOK_DEK=...` addition to that script and a worker restart. **Re-ran for real after the fix**: `step_type: "mcp"`, `tool_name: "mcp__zz_e2e_real_oauth__get_secret_fact"`, real tool output flowing into a real final model answer quoting the exact server-side secret fact, with the auth server's own independent log confirming 4 real token refreshes had already happened by that point (proving auto-refresh works transparently mid-flow, not just in the standalone refresh test). All test infrastructure (ngrok tunnel, standalone OAuth+MCP server, diagnostic scripts, the test MCP server row, the test agent) was torn down afterward via the real delete APIs — nothing test-related left behind in the repo or the database.
>
> **Separately found and fixed in the same pass — a real UI gap:** the agent builder's "saved servers" picker (`_mcp-server-picker.tsx`) fetched a stale `McpServerSummary` type that never carried `auth_type`/`oauth_status` (the API already returned both — only the frontend type was outdated), so a user could attach an OAuth server that was never actually connected, save the agent, and get the exact same silent "no tools" failure mode above with zero warning in the UI. Fixed: `McpServerSummary` now carries `auth_type`/`oauth_status`; the picker shows an amber warning badge + tooltip on any attached-but-unconnected OAuth server (both in the dropdown options and the selected-chip list) plus an inline banner pointing at the "Connect" button on the MCP Servers page.

> **BUILD STATUS (2026-07-11) — `tool_schemas` run-time read (§4/§9 "the scalability win") — ✅ SHIPPED, live-verified against a real third-party server.** Closes the last-remaining half of §4's original registry scope: the schema-refresh cron (M4 follow-up) has written `tool_schemas` since 2026-07-07, but the per-run adapter never read it — every registry-mode run still paid a fresh `connect()`+`listTools()` round trip, identical cost to inline mode, cache or no cache. Fixed: `mcp-registry.ts`'s `resolveRegistryMcpConfig` now selects `tool_schemas` and passes a non-empty cache through as `ResolvedMcpConfig.cachedTools`; `mcp.ts`'s `connectMcpTools` branches on it — when present, tools are namespaced/allow-listed/capped straight from the cache with **zero network calls**, and the real MCP connection now opens **lazily**, only if the model actually calls one of that server's tools (via a new `makeLazyClient` wrapper whose `close()` no-ops if nothing was ever opened, so a run that never calls a cached server's tools does zero MCP network work at all, not even a connect). A row with no cache yet (never swept) or `[]` falls through to the original connect-and-list path unchanged — no behavior change for it. One deliberate trade-off, stated plainly: a server that died since its last sweep now surfaces as a normal tool-call failure (§7 scenario 4, fed back as output) instead of being silently skipped at build time (§7 scenario 3) — the doc's own §6 design already signs off on this ("only opens a connection when a tool is actually called"). 10 new tests (8 in `mcp.test.ts` covering the lazy-connect semantics — no-connect-until-called, connects-at-most-once-across-multiple-calls, close-is-a-no-op-if-never-called, dead-server-fails-as-output-not-a-build-time-skip, SSRF/HTTPS guards still apply before advertising from cache, empty-cache-falls-back — plus 2 in `mcp-registry.test.ts` for the pass-through) — agent-runner now at 151/151 passing (up from 141), zero regressions, `tsc --noEmit` clean. **Live-verified against a real external server**, not just fixtures: ran the real, already-registered `deepwiki` curated server (3 cached tools from the live cron sweep) through a real kept agent — `step_type:"mcp"`, `tool_name:"mcp__deepwiki__ask_question"`, a real detailed grounded answer, the step's latency matching a single real DeepWiki round trip (the lazy connect + call combined), not a separate list-then-call sequence.

> **BUILD STATUS (2026-07-15) — independent re-verification (static + OAuth) + one real concurrency bug found and fixed.** Re-proved this whole feature live in a fresh session with no memory of the 07-07/07-11 work above, from scratch: registry-mode static call against the real curated `deepwiki` server (real grounded content, cached-tools lazy-connect confirmed), then a brand-new standalone OAuth 2.1 + MCP test server behind a fresh ngrok tunnel (the prior session's had long since been torn down) — real PKCE authorize/callback round trip, `oauth_status: connected`, real forced refreshes (20s access-TTL vs. the code's 60s skew means essentially every call refreshes). **New: ran two agents concurrently against the same OAuth server row** (the prior session's tests were all sequential) — this surfaced a real bug neither prior pass caught: both runs read the row before either had refreshed, both attempted a refresh with the same soon-to-be-stale token, one won (rotated the token, succeeded), the other's refresh was rejected by the provider — and the **loser's failure write landed after the winner's success write, stomping `oauth_status` back to `'error'` on a server that was, at that moment, healthy**. Confirmed it self-heals on the next successful call, so not a functional break, but a false-alarm window in the MCP Servers dashboard under real concurrent load. **Fixed** in `mcp-registry.ts`'s `persistOAuthState`: the failure-path write now carries a CAS guard (`.eq("oauth_refresh_token_enc", row.oauth_refresh_token_enc)`) so a losing racer's write only lands if the row hasn't already moved past the token that racer read — mirrors the same optimistic-concurrency pattern `settleSandboxSession` already uses for the sandbox (doc 13). One pre-existing test-mock gap found in the same pass: `mcp-oauth-refresh.integration.test.ts`'s `fakeSupabase` didn't support chaining a second `.eq()` after `.update()` (real supabase-js does) — fixed the mock, added a new regression test using a *stateful* fake that actually enforces the CAS condition (a non-stateful fake can't distinguish a real guard from a no-op one) to reproduce the exact race deterministically. Full suite: 154 passed / 5 skipped (Docker-gated, expected), `tsc --noEmit` clean. Re-ran the same concurrent-agents scenario live post-fix (post agent-runner restart) — both runs succeeded, `oauth_status` stayed `connected` throughout. Test infra (ngrok tunnel, standalone OAuth+MCP server) — left running at the user's explicit request so they could inspect the two new agents + the new MCP server registration in the dashboard themselves before teardown.
