...
import { POST } from '@/app/api/services/platform-apps/integrations/unlink/route';
import { mockUser } from '../../utils/mock-data';

vi.mock('@/lib/supabase/server');
vi.mock('@/lib/services/database-integration');

describe('POST /api/services/platform-apps/integrations/unlink', () => {
  const testUrl = 'http://localhost:3000/api/services/platform-apps/integrations/unlink';
  ...
  describe('Unlink Operation', () => {
    it('should return 404 when not linked', async () => {
      await mockAuthenticated();

      const { DatabaseIntegrationService } = await import('@/lib/services/database-integration');
      vi.mocked(DatabaseIntegrationService.unlink).mockResolvedValue({
        success: false,
        error: 'Not linked',
        code: 'NOT_LINKED',
      } as any);
    ...
    it('should unlink successfully with redeploy', async () => {
      await mockAuthenticated();

      const { DatabaseIntegrationService } = await import('@/lib/services/database-integration');
      vi.mocked(DatabaseIntegrationService.unlink).mockResolvedValue({
        success: true,
        removed_vars: ['DATABASE_URL', 'DATABASE_HOST'],
      ...
      expect(data.removed_vars).toContain('DATABASE_URL');
      expect(data.message).toContain('Redeploy triggered');
    });

    it('should unlink successfully without redeploy', async () => {
      await mockAuthenticated();

      const { DatabaseIntegrationService } = await import('@/lib/services/database-integration');
      vi.mocked(DatabaseIntegrationService.unlink).mockResolvedValue({
        success: true,
        removed_vars: ['DATABASE_URL'],
  ...
  describe('Error Handling', () => {
    it('should return 500 on unexpected error', async () => {
      ...
      const { DatabaseIntegrationService } = await import('@/lib/services/database-integration');
      vi.mocked(DatabaseIntegrationService.unlink).mockRejectedValue(new Error('Service down'));
