...
- [ ] Operator: schedule the Phase 11.C watchdog. Pick one path: (a) Cloudflare Cron Trigger on the existing inference worker hitting `POST https://wao.cs2hvh.com/api/inference/internal/serving-pod-watchdog` with header `X-Ahura-Internal-Token: $BATCH_PROCESSOR_TOKEN` every minute. (b) k8s CronJob on LKE doing the same `curl`. (c) anything else that fires every 60s. Without the watchdog, customer-stopped pods still tear down on `DELETE`; idle pods past `auto_stop_at` will linger until something invokes the endpoint. Recommended: option (a) since it's already in the same control plane and the Worker has the cron capability for free. Wrangler config snippet: add `[triggers]` `crons = ["* * * * *"]` then a `scheduled()` handler that fetches the URL.
