...
| 7.E | **SOC 2 readiness doc** — [`docs/inference/security.md`](./security.md). 16 sections covering the trust principles (confidentiality / integrity / availability / privacy), data inventory + retention, encryption posture (TLS 1.3 in transit + AES-256-GCM at rest for BYOK), authn/authz (key sha256 + RLS + role-based access), multi-tenancy isolation, audit logging, ZDR mode, webhook security (inbound + outbound HMAC-SHA256), spend controls, **the subprocessor list** (the one place customer-facing docs name Cloudflare/Supabase/OpenRouter/Linode/RunPod/Upstash/Resend/GitHub/HF — procurement legitimately needs this), incident response posture, vulnerability + patch management, and a candid SOC 2 audit-readiness gap list. Honest about what we are NOT (not certified, no HIPAA BAA, no FedRAMP). Designed for sales to hand to enterprise procurement teams. | ✅ |
