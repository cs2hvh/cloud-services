...
| 11.C | Auto-stop watchdog (a small worker that scans `serving_pod_auto_stop_at < NOW()` and tears down idle pods). Cold-start gateway gating so first request after start doesn't 504. Live request-count → extend `serving_pod_auto_stop_at` so active pods don't get killed. | TODO |
