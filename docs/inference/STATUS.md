...
| 11.D | **Dashboard activation button** on FT detail panel — replaces the manual SQL UPDATE. New API route `POST/DELETE /api/inference/fine-tuning/jobs/[id]/managed-serving` (admin/owner only, rate-limited, validates `serving_url` shape, flips both `inference.models.serving_url`/`is_managed` and the mirror on `inference.finetunes`). "Managed serving" section in the FT expanded row shows current state with a green "Active" dot when on, a "Disable managed" button. Off state shows a "Activate managed serving" button that opens a dialog with a single URL input + Enter-to-submit. The structured-log + audit-row layering is stubbed (no `managed.*` audit enum yet); fine until 11.D2 lands the audit + per-org pricing override. | ✅ |
