...
- **ROLLBACK WORKS, AND ONLY FROM A SCRIPT** 2026-08-27 (`418168fd`).
  ...
  - **THE ROUTE IS BLOCKED ON A CONVERGE PATH, and this is the decision to
    make.** Repointing an alias does not change what is served — the Ingress
    and Deployment are generated by the reconciler, and **there is no reconcile
    CronJob**: every sweep in this cluster reports drift rather than converging
    it. A route cannot call `reconcileProject` because it imports the
    service-role db client, which `boundary.test.ts` forbids under `app/`. So
    an API rollback today would write rows and keep serving the old version.
    Three ways out, none taken yet:
      1. Allowlist the route in `boundary.test.ts`. Cheapest, and it widens a
         security boundary for convenience — the exemption's own stated reason
         ("the caller cannot present a session") does not apply here.
      2. Refactor `reconcileProject` to take injected data instead of fetching
         it. Correct, and it touches the deploy path everything else depends on.
      3. A worker that converges queued intents, like the build worker does for
         deployments. Most consistent with what already exists; most work.
