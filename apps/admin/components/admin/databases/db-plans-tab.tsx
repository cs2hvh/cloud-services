...
export default function DbPlansTab({ all_products }: DbPlansTabProps) {
...
//   const [totalProducts, setTotalProducts] = useState(all_products.length);
  ...
  const getFilteredProducts = (source: Tables<"products">[] = productsList) => {
    ...
    // Apply type filter
    if (filterType !== "all") {
      filtered = filtered.filter((p) => p.sub === filterType);
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        toast.success("Database plan deleted successfully");
  ...
  const getDbTypeBadgeColor = (subtype: string) => {
    const colorMap: { [key: string]: string } = {
      pg: "bg-blue-950/50 text-blue-400 border-blue-900",
      mysql: "bg-orange-950/50 text-orange-400 border-orange-900",
      mongodb: "bg-green-950/50 text-green-400 border-green-900",
    };
    return colorMap[subtype] || "bg-white/[0.06] text-muted-foreground border-border";
  };
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="flex-1 flex gap-2">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Search by name, description, or database type..."
              value={searchQuery}
        ...
        <div className="flex gap-2">
          <Select value={filterType} onValueChange={handleFilterChange}>
            <SelectTrigger className="cursor-pointer w-[200px] bg-card border-border text-foreground focus:ring-0">
              <Filter className="h-4 w-4 mr-2" />
              <SelectValue placeholder="Filter by type" />
            </SelectTrigger>
            <SelectContent className="cursor-pointer bg-card border-border">
              {DB_TYPES.map((type) => (
                <SelectItem
                  key={type.value}
                  value={type.value}
                  className="cursor-pointer text-foreground focus:bg-white/[0.06] focus:text-foreground"
                >
                  {type.label}
                </SelectItem>
              ))}
                ...
                {products.length === 0 ? (
                  ...
                  products.map((product) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-foreground/80 max-w-[200px] truncate">
                          {product.description || (
                      ...
                      <td className="px-6 py-4">
                        <Badge
                          variant="outline"
                          className={`${getDbTypeBadgeColor(product.sub || "")}`}
                        >
                          {getDbTypeDisplay(product.sub || "")}
                        </Badge>
                      </td>
