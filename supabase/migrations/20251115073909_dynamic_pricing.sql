...
DO $$ BEGIN
  CREATE POLICY "Allow public read access to active pricing categories"
      ON pricing_categories FOR SELECT USING (active = true);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  CREATE POLICY "Allow admins to manage pricing categories"
      ON pricing_categories FOR ALL
      USING (auth.uid() IN (SELECT id FROM user_profiles WHERE roles @> ARRAY['admin'::user_role]))
      WITH CHECK (auth.uid() IN (SELECT id FROM user_profiles WHERE roles @> ARRAY['admin'::user_role]));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
DO $$ BEGIN
  CREATE POLICY "Allow public read access to active pricing promos"
      ON pricing_promos FOR SELECT USING (active = true);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  CREATE POLICY "Allow admins to manage pricing promos"
      ON pricing_promos FOR ALL
      USING (auth.uid() IN (SELECT id FROM user_profiles WHERE roles @> ARRAY['admin'::user_role]))
      WITH CHECK (auth.uid() IN (SELECT id FROM user_profiles WHERE roles @> ARRAY['admin'::user_role]));
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
-- =========================================
