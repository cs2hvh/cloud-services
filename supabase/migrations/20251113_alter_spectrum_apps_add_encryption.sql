...
-- 1. Rename existing primary key column id -> spectrum_id
ALTER TABLE spectrum_apps RENAME COLUMN id TO spectrum_id;

-- 2. Add new surrogate primary key id (UUID)
ALTER TABLE spectrum_apps ADD COLUMN id UUID DEFAULT uuid_generate_v4();
...
ALTER TABLE spectrum_apps ADD COLUMN hostname_enc JSONB;
