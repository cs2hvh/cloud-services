-- Fix compatibility for hourly billing atomic lookup across environments where
-- billing active tables may be keyed by either service_id or id.
...
BEGIN
  ...
  -- Prefer service_id lookup, fallback to id for legacy environments.
  SELECT CASE
    WHEN EXISTS (
      SELECT 1
  ...
  -- Lock active service row and read current billing pointer.
  EXECUTE format(
  ...
  -- Prevent duplicate billing when concurrent workers use stale snapshots.
  ...
  -- Preserve existing flow: advance last_billed_at first.
  EXECUTE format(
  ...
  -- Then attempt to deduct credit atomically.
