...
-- Drop existing service type constraint on audit_logs
ALTER TABLE audit_logs 
  DROP CONSTRAINT IF EXISTS audit_logs_service_type_check;

-- Add updated service type constraint for audit logs
ALTER TABLE audit_logs
...
-- Add index for AI agent and knowledge base audit logs
CREATE INDEX IF NOT EXISTS idx_audit_logs_ai_services 
  ON audit_logs(user_id, service_type, created_at DESC) 
  WHERE service_type IN ('ai_agent', 'knowledge_base');

-- Add index for service_id lookups in audit logs
...
COMMENT ON CONSTRAINT audit_logs_service_type_check ON audit_logs IS 
